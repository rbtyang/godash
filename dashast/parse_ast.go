...
func (a *Ast) ParseFile(inputPath string) (err error) {
	...
	if err != nil {
		dashlog.Errorf("ParseFile ParseFile err:%v", err)
		return
	}
	...
	if err != nil {
		dashlog.Errorf("ParseFile ParseScope err:%v", err)
		return
	}

	err = a.ParseDecls(f.Decls)
	if err != nil {
		dashlog.Errorf("ParseFile ParseDecls err:%v", err)
		return err
	}
...
func (a *Ast) ParseInterface(typ *ast.TypeSpec) (ret Interface) {
	...
	for _, method := range it.Methods.List {
		...
		if ok != true {
			dashlog.Panicf("interface just support function")
		}
		funcT := a.ParseFunc(f)
